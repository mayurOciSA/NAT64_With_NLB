...
resource "oci_core_instance" "nat66_backend" {
  ...
  create_vnic_details {
    ...
    assign_ipv6ip             = true # Assign IPv6 address from subnet's IPv6 CIDR block TODO: Add more IPv6 GUAs for scale, also update nat66.setup.sh accordingly or one may need Ansible to manage that
...
output "backend_nat66_private_ipv6s" {
  value = data.oci_core_ipv6s.backend_nat66_private_ipv6.ipv6s[*]
}

output "backend_nat66_private_ipv4s" {
  value = data.oci_core_private_ips.backend_nat66_private_ipv4.private_ips[*]
}

# TODO
# resource "local_file" "rendered_cloud_init_nat66" {
#   filename = "${path.module}/nat66/cloud-init.rendered.yaml"
#   content = templatefile("${path.module}/nat66/cloud-init.yaml", {
#     nat66_script = file("${path.module}/nat66/nat66.setup.sh")
#   })
# }
...
resource "terraform_data" "provision_nat66_backend" {
  count      = var.backend_nat66_count
  depends_on = [oci_core_instance.nat66_backend, terraform_data.SOCK5_tunnel_start]
  ...
  provisioner "local-exec" {
    ...
    command = <<EOT
        ...
        scp ${local.ssh_proxy_options} -i ${var.ssh_private_key_local_path} ${path.module}/nat66/nat66.install.sh opc@$nat66ipv4host:~/nat66.install.sh
        if [ $? -ne 0 ]; then
          echo "SCP failed for NAT66"
          exit 1
        fi

        ssh ${local.ssh_proxy_options} ${local.ssh_proxy_options}  \
          opc@$nat66ipv4host \
          "sudo bash ~/nat66.install.sh" 2>&1 | tee ${path.module}/nat66/nat66.install.$nat66ipv4host.log
        rc=$?
        exit $rc
    EOT
  }
